...
# SVM (NMF) Multiclass 
print "One Vs One Classification using SVM (NMF)"
one_vs_one = OneVsOneClassifier(svm.SVC(kernel='linear')).fit(X_train_nmf, four_train.target)
predicted = one_vs_one.predict(X_test_nmf)
print_stats(four_test.target, predicted)

print "One Vs Rest Classifciation using SVM (NMF)"
one_vs_rest = OneVsRestClassifier(svm.SVC(kernel='linear')).fit(X_train_nmf, four_train.target)
predicted = one_vs_rest.predict(X_test_nmf)
print_stats(four_test.target, predicted)

# SVM (SVD) Multiclass 
print "One Vs One Classification using SVM (SVD)"
one_vs_one = OneVsOneClassifier(svm.SVC(kernel='linear')).fit(X_train_lsi, four_train.target)
predicted = one_vs_one.predict(X_test_lsi)
print_stats(four_test.target, predicted)

print "One Vs Rest Classifciation using SVM (SVD)"
one_vs_rest = OneVsRestClassifier(svm.SVC(kernel='linear')).fit(X_train_lsi, four_train.target)
predicted = one_vs_rest.predict(X_test_lsi)
print_stats(four_test.target, predicted)
